    ...
    def batch_encode_alleles(self, alleles: List[str], batch_size: int = 8, force: bool = False) -> Dict[str, np.ndarray]:
        ...
        for allele in alleles:
            ...
            if not force and std_allele in self.embeddings:
                ...
                sequence = self.get_sequence(std_allele) # Use standardized name for lookup
                if sequence:
                    alleles_to_encode.append(std_allele) # Store standardized name
                    sequences_to_encode.append(sequence)
                else:
                    logger.warning(f"No sequence found for allele {allele} (standardized: {std_allele})")
                    missing_sequences.append(allele) # Store original allele name

        # Batch encode the sequences that need encoding
        if sequences_to_encode:
            logger.info(f"Batch encoding {len(sequences_to_encode)} sequences with ProtBERT...")
            try:
                # Call the ProtBERT-specific batch_encode method
                encoded_embeddings = self.batch_encode(sequences_to_encode, batch_size=batch_size)

                # Add new embeddings to results and cache using standardized allele names
                for i, std_allele in enumerate(alleles_to_encode):
                    results[std_allele] = encoded_embeddings[i]
                    self.embeddings[std_allele] = encoded_embeddings[i]

                # Save updated cache
                self._save_embedding_cache()

            except Exception as e:
                logger.error(f"Error during ProtBERT batch encoding: {e}")
                # Add alleles that failed encoding to missing list
                # Find original allele names corresponding to failed std_alleles
                original_failed_alleles = [a for a in alleles if self._standardize_allele(a) in alleles_to_encode]
                missing_sequences.extend(original_failed_alleles)

        if missing_sequences:
            logger.warning(f"Failed to encode {len(missing_sequences)} alleles: {missing_sequences}")

        # Ensure the final dictionary uses the original requested allele names as keys
        final_results = {}
        for requested_allele in alleles:
             std_allele = self._standardize_allele(requested_allele)
             if std_allele in results:
                 final_results[requested_allele] = results[std_allele]
             # else: it was missing, already logged
